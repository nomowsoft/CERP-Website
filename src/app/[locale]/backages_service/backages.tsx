            ...
            <div className="flex flex-wrap justify-center items-center gap-8 lg:gap-10 lg:mx-0 mx-4">
                {Packeges.map((packege: PackageDTO, index: number) => (
                    <div 
                        ...
                        className="h-full w-full md:w-[400px] lg:w-[420px]"
                    >
                            ...
                            <div className="relative z-10 flex-1">
                                <div className="flex flex-col items-center text-center mb-6">
                                    ...
                                    <div className="flex items-center gap-2 mb-4">
                                        <span className={`inline-block px-3 py-1 text-sm font-bold rounded-lg ${index === 1 ? 'bg-primary/10 text-primary' : 'bg-gray-100 text-gray-600'}`}>
                                            {packege.type}
                                        </span>
                                        {mySubscription?.packageId === packege.id && (
                                            <span className="inline-block px-3 py-1 text-sm font-bold rounded-lg bg-emerald-100 text-emerald-600 animate-pulse">
                                            ...
                                            <div className="flex flex-wrap justify-center gap-4">
                                                {packege.systems.map((system: any) => (
                                                    <div 
                                                        ...
                                                        className="group/system relative flex flex-col items-center gap-1.5 cursor-pointer"
                                                    >
                                                        <div className="w-12 h-12 rounded-2xl bg-gray-50 group-hover/system:bg-primary/5 border border-gray-100 group-hover/system:border-primary/20 flex items-center justify-center p-2.5 transition-all duration-300">
                                                            {system.icon ? (
                                                                <Image src={system.icon} alt="" width={28} height={28} className="object-contain transition-transform group-hover/system:scale-110" />
                                                            ) : (
                                                                <Settings2 className="w-6 h-6 text-gray-300 group-hover/system:text-primary/40" />
                                                            )}
                                                        </div>
                                                        <span className="text-[10px] font-bold text-gray-500 group-hover/system:text-primary transition-colors">
                        ...
                        {filteredSystems.map((system: any, index: number) => (
                            <div key={system.id} data-aos="fade-up" data-aos-delay={index * 100} className="h-full">
                                <div className={`group h-full bg-white rounded-[2.5rem] p-8 md:p-10 border border-gray-100 shadow-lg transition-all duration-500 relative overflow-hidden flex flex-col justify-between ${selectedSystems.includes(system.id) ? 'border-secondary' : 'hover:border-primary/30'}`}>
                                    <button 
                                        onMouseEnter={() => setSelectedSystem(system)}
                                        className="absolute top-8 right-8 p-2.5 rounded-2xl bg-gray-50 text-gray-400 hover:text-primary hover:bg-primary/5 transition-all duration-300 z-20"
                                    >
